...
def extract_go_routes(service_name: str, port: int, source_dir: str) -> list:
    ...
    for go_file in sorted(source_path.rglob("*.go")):
        ...
        # Extract mux routes
        method_map = {}
        for match in MUX_METHOD_RE.finditer(content):
            path, method = match.groups()
            method_map[path] = method

        for match in MUX_ROUTE_RE.finditer(content):
            var_name, path, handler = match.groups()
            method = method_map.get(path, "GET")
            line_num = content[:match.start()].count("\n") + 1
